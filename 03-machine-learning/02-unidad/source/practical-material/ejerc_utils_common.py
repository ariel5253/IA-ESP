""" 
ejerc_utils_common.py
    functions common to all optional labs, Course 1, Week 2 
"""
""" 
ejerc_utils_common.py
    funciones comunes a todos los ejercicios opcionales, Curso 1, Semana 2 
"""
...
def compute_cost_matrix(X, y, w, b, verbose=False):
    ...
    Calcula el gradiente para regresión lineal
     Argumentos:
      X (ndarray (m,n)): Datos, m ejemplos con n X entradas (características)
      y (ndarray (m,)) : valores objetivo
      w (ndarray (n,)) : parámetros del modelo
      b (escalar)      : parámetro del modelo
      verbose : (Boolean) Si es True, imprime el valor intermedio f_wb
    Retorna
      costo: (escalar)
    """
    m = X.shape[0]
...
def compute_gradient_matrix(X, y, w, b):
    ...
    # Versión con bucle de compute_cost multivariable
...
def compute_gradient(X, y, w, b):
    ...
    Argumentos:
      ...
      b (escalar)      : parámetro del modelo
    Retorna
      dj_dw (ndarray forma(shape) (n,)): El gradiente del costo respecto a los parámetros w.
      dj_db (escalar):                  El gradiente del costo respecto al parámetro b.
    """
    m,n = X.shape           #(número de ejemplos, número de X entradas (características))
    dj_dw = np.zeros((n,))
    dj_db = 0.
