...
def plot_kMeans_RGB(X, centroids, idx, K):
    fig = plt.figure(figsize=(16, 16))
    ax = fig.add_subplot(111, projection='3d')  # un solo subplot

    ax.scatter(*(X.T * 255), depthshade=False, s=.3, c=X)
    ax.scatter(*(centroids.T * 255), depthshade=False, s=500,
    ...
    for axis in (ax.xaxis, ax.yaxis, ax.zaxis):
        if hasattr(axis, 'set_pane_color'):
            axis.set_pane_color((0., 0., 0., .2))
        elif hasattr(axis, 'pane') and hasattr(axis.pane, 'set_facecolor'):
            axis.pane.set_facecolor((0., 0., 0., .2))
