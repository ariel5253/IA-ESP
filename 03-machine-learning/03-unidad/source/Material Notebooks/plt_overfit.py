...
from lab_utils_common import np, gradient_descent

def train_logistic(X, y, alpha=0.1, num_iters=10000, lambda_=0.0):
    """
    Train logistic regression via gradient descent.
    X: (m×n) feature matrix, y: (m,) labels in {0,1}
    Returns w:(n×1), b:scalar, losses:list
    """
...
def train_ridge(X, y, alpha=0.01, num_iters=10000, lambda_=1.0):
    """
    Train linear (ridge) regression via gradient descent.
    X: (m×n) feature matrix, y: (m,) continuous labels
    Returns w:(n×1), b:scalar, losses:list
    """
    w = np.zeros((X.shape[1], 1))
    b = 0.0
...
class overfit_example():
    ...
    def linear_regression(self):
        ...
        self.X_mapped_scaled, self.X_mu, self.X_sigma = (
            zscore_normalize_features(self.X_mapped)
        )
    
        # 3) train ridge via gradient descent
        w_mat, b_scalar, _ = train_ridge(
            self.X_mapped_scaled,
            self.y,
            alpha=0.01,
            num_iters=10000,
        ...
        # 4) flatten & assign
        self.w = w_mat.reshape(-1,)
        self.b = b_scalar
    
        # 5) prepare plot points
        x_plot = np.linspace(*self.xlim, 30)
        xm, _ = map_one_feature(x_plot, self.degree)
        xms = (xm - self.X_mu) / self.X_sigma
    
        # 6) predict and draw
        y_pred = xms.dot(w_mat) + b_scalar
        self.linear_data(redraw=True)
        self.ax0yfit = self.ax[0].plot(
            x_plot, y_pred, color="blue", label="y_fit"
        )
        self.ax0ledgend = self.ax[0].legend(loc='lower right')
        self.fig.canvas.draw()
    ...
    def logistic_regression(self):
        ...
        # 2) map & normalize
        self.X_mapped, _ = map_feature(self.X[:,0], 
                                       self.X[:,1], 
                                       self.degree)
        self.X_mapped_scaled, self.X_mu, self.X_sigma = (
            zscore_normalize_features(self.X_mapped)
        ...
        # 3) choose regularization
        lam = self.lambda_ if self.regularize else 0.0
    
        # 4) train via gradient descent
        w_mat, b_scalar, _ = train_logistic(
            self.X_mapped_scaled,
            self.y,
            alpha=0.1,
            num_iters=10000,
