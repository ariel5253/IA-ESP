#!/usr/bin/env python3
"""
Generador Automático de Informe EDA Completo
============================================

Este script ejecuta automáticamente:
1. El notebook de análisis EDA
2. Genera el informe en formato Markdown
3. Convierte a formato DOCX
4. Limpia archivos temporales

Uso: python generar_informe_completo.py
"""
...
def print_banner():
    ...
    print("=" * 80)
    print("🧠 GENERADOR AUTOMÁTICO DE INFORME EDA - CORHUILA")
    print("=" * 80)
...
def execute_analysis():
    """Ejecutar el análisis EDA completo"""
    print("🚀 Ejecutando análisis EDA...")
    
    try:
        # Código del análisis EDA completo
        exec("""
...
*Informe elaborado mediante técnicas de análisis descriptivo exploratorio (EDA)*"""
    ...
    try:
        with open('informe_eda_auto.md', 'w', encoding='utf-8') as f:
            f.write(markdown_content)
        print("✅ Informe Markdown generado: informe_eda_auto.md")
        return True
    except Exception as e:
...
def generate_docx_report():
    ...
    try:
        ...
        with open('informe_eda_auto.md', 'r', encoding='utf-8') as f:
            md_content = f.read()
        ...
        # Guardar documento
        doc.save('informe_eda_auto.docx')
        print("✅ Informe DOCX generado: informe_eda_auto.docx")
        return True
...
def show_results():
    ...
    if os.path.exists('informe_eda_auto.md'):
        size = os.path.getsize('informe_eda_auto.md') / 1024
        files_generated.append(f"📝 informe_eda_auto.md ({size:.1f} KB)")
    
    if os.path.exists('informe_eda_auto.docx'):
        size = os.path.getsize('informe_eda_auto.docx') / 1024
        files_generated.append(f"📄 informe_eda_auto.docx ({size:.1f} KB)")
...
def main():
    ...
    if not execute_analysis():
        print("❌ Error en la ejecución del análisis")
        return
